Template.calendarAppLazy.rendered = function (){
        var AJAX = null;
        var selectedEvent;
        $('#calendarLazy').pagescalendar({
            ...
            onViewRenderComplete:function(range){
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $("#calendarLazy").pagescalendar("setState","loaded");
                        $("body").removeClass('pending');
                        $("#calendarLazy").pagescalendar("removeAllEvents");
                        $("#calendarLazy").pagescalendar("addEvents",data);
                    },
                    error: function (ajaxContext) {
                        $("#calendarLazy").pagescalendar("error",ajaxContext.status+": Something horribly went wrong :(" );
                        $("body").removeClass('pending');
                    }
                });
            },
            ...
            onTimeSlotDblClick: function(timeSlot) {
                ...
                $('#calendarLazy').pagescalendar('addEvent', newEvent);
                setEventDetailsToForm(selectedEvent);
            },
            onDateChange:function(range){
                ...
                $("#calendarLazy").pagescalendar("setState","loading");
                $.ajax({
                    type: "POST",
                    url: "http://revox.io/json/events.json",
                    data: "",
        ...
        $('#eventSave').on('click', function() {
            ...
            $('#calendarLazy').pagescalendar('updateEvent',selectedEvent);

            $('#calendar-event').removeClass('open');
        });

        $('#eventDelete').on('click', function() {
            $('#calendarLazy').pagescalendar('removeEvent', $('#eventIndex').val());
            $('#calendar-event').removeClass('open');
        });
};